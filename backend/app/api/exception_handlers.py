...
def classify_exception(exc: Exception) -> tuple[int, str, str]:
    """Returns the shared transport classification for JSON and stream errors."""
    if isinstance(exc, QueryExecutionException):
        return 400, "QUERY_EXECUTION_ERROR", "The query could not be executed against the database."
    if isinstance(exc, SQLSafetyViolation):
        return 400, "SQL_VALIDATION_ERROR", "The generated SQL statement failed safety validation."
    if isinstance(exc, SQLServiceException):
        return (
            400,
            "SQL_GENERATION_ERROR",
            "The LLM could not generate a valid SQL statement for this question.",
        )
    if isinstance(exc, ReportServiceException):
        return (
            502,
            "LLM_ERROR",
            "The language model failed to generate the report. Please try again.",
        )
    if isinstance(exc, WorkflowServiceException):
        return (
            500,
            "WORKFLOW_ERROR",
            "An internal workflow error occurred. The request could not be completed.",
        )
...
async def handle_query_execution_exception(
    ...
    return _error_response(
        400, "QUERY_EXECUTION_ERROR", "The query could not be executed against the database."
    )


async def handle_sql_safety_violation(request: Request, exc: SQLSafetyViolation) -> JSONResponse:
    """Maps SQLSafetyViolation → HTTP 400."""
    logger.warning(f"SQLSafetyViolation on {request.url.path}: {exc}")
    return _error_response(
        400, "SQL_VALIDATION_ERROR", "The generated SQL statement failed safety validation."
    )


async def handle_sql_service_exception(request: Request, exc: SQLServiceException) -> JSONResponse:
...
async def handle_report_service_exception(
    ...
    return _error_response(
        502, "LLM_ERROR", "The language model failed to generate the report. Please try again."
    )
...
async def handle_app_base_exception(request: Request, exc: AppBaseException) -> JSONResponse:
